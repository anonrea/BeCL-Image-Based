...
class BECLAgent:
    def __init__(self, obs_shape, action_shape, device, lr, feature_dim,
                 ...
                 contrastive_update_rate, temperature, skill, skill_dim, update_skill_every_step, reward_free,
                 **kwargs):
        self.device = device
        ...
        self.actor = Actor(self.encoder.repr_dim + self.skill_dim, action_shape, feature_dim,
                           hidden_dim).to(device)
        
        # net
        self.becl = BECL(self.encoder.repr_dim,
                                           self.skill_dim,
                                           hidden_dim).to(device)

        # optimizers
        self.becl_opt = torch.optim.Adam(self.becl.parameters(), lr=lr)
        
        self.critic = Critic(self.encoder.repr_dim + self.skill_dim, action_shape, feature_dim,
                             hidden_dim).to(device)
        self.critic_target = Critic(self.encoder.repr_dim + self.skill_dim, action_shape,
    ...
    def act(self, obs, meta, step, eval_mode):
        ...
        h = self.encoder(obs.unsqueeze(0))
        inputs = [h]
    ...
    def update(self, replay_iter, step):
        ...
        aug_obs = torch.cat([aug_obs, skill], dim=1)
